...
namespace SimpleGE
{
  ...
  public:
    ...
    struct SetupResult
    {
      ...
      SetupResult(SetupResult&& other) : comp(other.comp), steps(std::move(other.steps)), current(other.current)
      {
        other.current = 0;
      }
      ...
      bool IsDone() const { return current == steps.size(); }

      bool Ready() const { return steps[current].ready(); }
    ...
    [[nodiscard]] static bool Resolve(std::vector<SetupResult>& pending)
    {
      while (!pending.empty())
      {
        ...
        incomplete.reserve(pending.size());
        bool anyReady{};
