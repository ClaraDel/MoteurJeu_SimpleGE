...
namespace SimpleGE
{
  static void ReadPngData(png_structp png_ptr, png_bytep outBytes, png_size_t byteCountToRead)
  {
    ...
    auto* reader = reinterpret_cast<std::vector<unsigned char>::const_iterator*>(io_ptr);
  ...
  Image::Image(const std::vector<std::byte>& content)
  {
    ...
    int bitDepth;
    int colorType;
    auto retVal = png_get_IHDR(png_ptr, info_ptr, &width, &height, &bitDepth, &colorType, nullptr, nullptr, nullptr);
    Ensures(retVal != -1);
    ...
    m_Content.resize(height * bytesPerRow / sizeof(PixelRGBA8));
    std::vector<PixelRGBA8*> rowPointers(height);
    for (int y = 0; y < height; y++)
    {
      rowPointers[y] = &m_Content[y * bytesPerRow / sizeof(PixelRGBA8)];
    }
