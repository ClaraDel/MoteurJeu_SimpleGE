...
namespace SimpleGE
{
  ...
  static const std::unordered_map<std::string, KeyAPIType> KeyboardKeys = {
  ...
  static void from_json(const json& j, InputComponent::Description& desc)
  {
  ...
  InputComponent::InputComponent(Entity& owner) : Component(owner), impl(std::make_unique<InputImpl>()) {}

  InputComponent::~InputComponent() = default;

  Component::SetupResult InputComponent::Setup(const nlohmann::json& descr) { return Setup(descr.get<Description>()); }

  Component::SetupResult InputComponent::Setup(const Description& descr)
  {
    for (const auto& action : descr.mapping)
    {
      auto kbMap = KeyboardKeys.find(action.second);
      impl->mapping[action.first] = kbMap->second;
    }

    return {this, {}};
  }

  [[nodiscard]] bool InputComponent::IsActive(Action action) const
  {
    gsl::not_null<CameraComponent*> currentCamera = CameraComponent::Current();
    return glfwGetKey(&currentCamera->GetNativeWindow(), impl->mapping[action]) == GLFW_PRESS;
  }
} // namespace SimpleGE
