...
namespace SimpleGE
{
  ...
  static void from_json(const json& j, SpriteSheetImpl& impl)
  {
    j.at("frames").get_to(impl.sprites);
    j.at("meta").get_to(impl.meta);

    const float invScaleH = 1.F / float(impl.meta.size.width);
    const float invScaleV = 1.F / float(impl.meta.size.height);

    for (auto& f : impl.sprites)
    {
  ...
  SpriteSheetComponent::SpriteSheetComponent(Entity& owner)
      : TextureComponent(owner), Component(owner), impl(std::make_unique<SpriteSheetImpl>())
  {
  }

  SpriteSheetComponent::~SpriteSheetComponent() = default;
  ...
  Component::SetupResult SpriteSheetComponent::Setup(const Description& descr)
  {
    ...
    json::parse(content->Value()).get_to(*impl);
  ...
  {
    const auto item = impl->sprites.find(std::string{key});
    if (item != impl->sprites.end())
    {
      return &item->second;
    }
    return nullptr;
